...
public class ThumbnailView extends View {
    ...
    public void addArrow(String startPointName, String endPointName) {
        ...
        if (startPoint != null && endPoint != null) {
            Arrow arrow = new Arrow(startPoint, endPoint, Color.BLUE); // You can set arrow color as per your requirement
            addArrow(arrow);
        }
    }
    ...
    private void init() {
        mPaint = new Paint();
        mPaint.setColor(ContextCompat.getColor(this.getContext(), R.color.yellow));
        mPaint.setStyle(Paint.Style.STROKE);
        mPaint.setStrokeWidth(3); // Set stroke width as needed
        mFrameRect = new RectF();

        yPaint = new Paint();
        yPaint.setColor(Color.RED); // Red color
        yPaint.setStyle(Paint.Style.STROKE);
        yPaint.setStrokeWidth(3); // Set stroke width as needed
    ...
    public void updateArrow(Context context) throws ExecutionException, InterruptedException {
    ...
    public void updateFlag(Context context) throws ExecutionException, InterruptedException {
        DBhelper dbHelper = new DBhelper(context);
        Map<String, Integer> orderMap = dbHelper.getAllPlaceOrders(); // 获取所有景点及其顺序的映射
