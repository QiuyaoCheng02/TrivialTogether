...
public class PinButtonView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (PinButton pinButton : pinButtonList) {
            ...
            if(pinButton.getName().equals(AttractionsInfo.LEIFENGTA)||pinButton.getName().equals(AttractionsInfo.QINGHEFANG)||pinButton.getName().equals(AttractionsInfo.CHENGHUANGGE)
            ||pinButton.getName().equals(AttractionsInfo.FENGHUANGSI)||pinButton.getName().equals(AttractionsInfo.XIHU)){
                float buttonWidth = 200 * mScale;
                float buttonHeight = 100 * mScale;

                // Draw button at the calculated position with adjusted size
                drawButton(canvas, viewX, viewY, buttonWidth, buttonHeight, pinButton.getColor(), 0);
            } else if (pinButton.getName().equals(AttractionsInfo.GUQIANG)||pinButton.getName().equals(AttractionsInfo.GEDE)||pinButton.getName().equals(AttractionsInfo.WENDEMU)) {
                float buttonWidth = 400 * mScale;
                float buttonHeight = 100 * mScale;
                drawButton(canvas, viewX, viewY, buttonWidth, buttonHeight, pinButton.getColor(), 0);
            } else{
                // Calculate button size relative to the image scale
                float buttonWidth = 250 * mScale;
                float buttonHeight = 100 * mScale;
