...
public class PinButtonView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (PinButton pinButton : pinButtonList) {
            ...
            if(pinButton.getName().equals(AttractionsInfo.MINGGUGONG)||pinButton.getName().equals(AttractionsInfo.YINGZHOU)||pinButton.getName().equals(AttractionsInfo.JIMINGSI)){
                float buttonWidth = 200 * mScale;
                float buttonHeight = 100 * mScale;
            ...
            }else if (pinButton.getName().equals(AttractionsInfo.JINIANGUAN)){
                // Calculate button size relative to the image scale
                float buttonWidth = 600 * mScale;
                float buttonHeight = 100 * mScale;
    ...
    private void drawButton(Canvas canvas, float x, float y, float width, float height, int color, int alpha) {
        ...
        canvas.drawRect(x, y, x + width, y + height, paint); // Draw a button rectangle
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        // 如果触摸点不在按钮范围内，将触摸事件传递给 LargeBitmapView
        if (largeBitmapView != null) {
            largeBitmapView.onTouchEvent(event);
        }
